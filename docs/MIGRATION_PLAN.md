...
7. **Phase 5 — Vectorized DVM/technical modules**: `dvm_global.py` first (cleanest, most direct port — pivot→2D matrix, `rolling().mean()`→moving-average kernel, `ewm()`→exponential recursion; match pandas' `ewm(adjust=...)` default explicitly), then `dvm_engine.py` (port `ProcessPoolExecutor` fan-out to a C++ thread pool), then `dvm_composite.py`. Preserve — do not silently "fix" — the documented snapshot-not-PIT limitation of these modules' fundamentals inputs. *(`dvm_global.py` done — 18/18 new assertions across 4 test cases (58 in `bt_dvm_global_tests`, plus a corrected `mfi()` golden test in `bt_stats_tests`); `dvm_engine.py`/`dvm_composite.py` still pending. See "Phase 5 notes" below for the divergences deliberately preserved.)*

   **Phase 5 notes (`dvm_global.py` → `libs/quant_core/dvm_global`):** `dvm_global.py` defines two implementations of the same metrics — a per-ticker loop (`_tech()`) and the vectorised `process_market()` actually invoked by `main()`'s `ProcessPoolExecutor` fan-out. This port replicates `process_market()`'s exact behavior where the two diverge from each other or from the module's own docstring:
   ...
   - The `dist_52w` momentum sub-score (`100 + ((px/hi52-1)*100)*3`) is a different formula from the already-ported `dvm_engine.py` `momentum_score()`'s 52w-high sub-score (`100-(hi52/px-1)*300`) — not algebraically equivalent, not consolidated.
